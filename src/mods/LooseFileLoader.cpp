...
void LooseFileLoader::on_draw_ui() {
    ...
    if (m_hook_success) {
        ...
        if (ImGui::TreeNode(_("Debug"))) {
            ImGui::Checkbox(_("Enable file cache"), &m_enable_file_cache);
            ImGui::TextWrapped(_("Cache hits: %d"), m_cache_hits);
            ImGui::TextWrapped(_("Uncached hits: %d"), m_uncached_hits);

            if (ImGui::Button(_("Clear existence cache"))) {
                clear_existence_cache();
            }
